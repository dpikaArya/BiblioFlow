"""AIBEF Complete End-to-End Execution & Certification Script.

Runs the full pipeline on WoS TXT files from ~/Desktop/CR,
...
OUTPUT_DIR = Path(__file__).parent / "outputs" / "certification_run"
TIMESTAMP = datetime.now().strftime("%Y%m%d_%H%M%S")

INPUT_FILES = ["0.txt", "1.txt", "2.txt", "3.txt", "4.txt", "5.txt"]
...
def _gen_certification_report(work_dir, stages, r_cert, mv_result):
    ...
    doc.add_paragraph(f"Input: {len(INPUT_FILES)} WoS TXT files from ~/Desktop/CR")
...
def _gen_replay_report(work_dir, state):
    ...
    doc.add_paragraph("Pipeline executed deterministically on all 6 WoS TXT files.")
...
def _gen_framework_summary(work_dir, state, stages, r_cert, mv_result):
    ...
    doc.add_paragraph(f"Generated: {datetime.now().isoformat()}")
    doc.add_paragraph(f"Run ID: cert_{TIMESTAMP}")
...
def print_summary(tracker: ExecutionTracker, state: PipelineState):
    ...
    r_all_pass = r.get("convert2df", {}).get("success") and r.get("biblioAnalysis", {}).get("success") and r.get("summary", {}).get("success")
    cert = "READY_FOR_BIBLIOSHINY" if (all_stages_pass and r_all_pass) else "NOT_CERTIFIED"

    total_time = sum(s["duration"] for s in tracker.results.values())
    r_time = sum(float(r.get(k, {}).get("elapsed", 0)) for k in ["convert2df", "biblioAnalysis", "summary"] if r.get(k, {}).get("success"))
...
def main():
    print(f"AIBEF Certification Run - {TIMESTAMP}")
